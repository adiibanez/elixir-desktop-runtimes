...
<% 
config = "--with-ssl=/usr/local/openssl/ --disable-dynamic-ssl-lib --without-javac --without-odbc --without-wx --without-debugger --without-observer --without-cdv --without-et --without-megaco --xcomp-conf=xcomp/erl-xcomp-#{@arch.id}-android.conf #{System.get_env("KERL_CONFIGURE_OPTIONS", "")}"
