...
# Fetching OTP
#RUN git clone --depth 1 <%= @otp_source %> _build/otp_cache/otp --branch <%= @otp_tag %>
COPY _build/otp_cache/otp_src_<%= @otp_tag %>.tar.gz ./
RUN tar -xzf otp_src_<%= @otp_tag %>.tar.gz \
    && mv otp_src_<%= @otp_tag %> otp \
    && rm otp_src_<%= @otp_tag %>.tar.gz
...
<% 
config = "--with-ssl=/usr/local/openssl/ --disable-dynamic-ssl-lib --without-javac --without-odbc --without-wx --without-debugger --without-observer --without-cdv --without-et --xcomp-conf=xcomp/erl-xcomp-#{@arch.id}-android.conf #{System.get_env("KERL_CONFIGURE_OPTIONS", "")}"
